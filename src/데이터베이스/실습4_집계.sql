...
select * from books; 
select avg( price ) as 평균단가 from books as 도서; 
...
select genre, count(genre) from books group by genre; -- 결과는 같음
select genre, count(*) from books group by genre; -- 결과는 같음
...
select sum(order_qty) from orders where customer = '김민준' or customer = '이서연'; -- 두사람 합계 총 주문 수량
select sum(order_qty) from orders where customer in('김민준', '이서연') ; -- 두사람 합계 총 주문 수량

select customer, sum(order_qty) as 총주문수량 from orders where customer = '김민준' having 총주문수량 >= 4; 
select customer, sum(order_qty) as 총주문수량 from orders where customer = '김민준' or customer = '이서연' having 총주문수량 >= 4;  -- //안됨ㅜㅜ

select customer, sum(order_qty) as 총주문수량 from orders group by customer; 
select customer, sum(order_qty) as 총주문수량 from orders group by customer having 총주문수량 >= 4 limit 2; -- 나오긴 함...ㅋㅋㅋ

select customer, sum(order_qty) 총주문수량 from orders where customer in('김민준', '이서연') group by customer having 총주문수량 >= 4; -- 답안 : 두사람 합계 총 주문 수량
select customer, sum(order_qty) 총주문수량 
from orders 
where customer in('김민준', '이서연') 
group by customer 
having 총주문수량 >= 4; 
...
select avg( price ) 평균가격 from books where stock < 5 ;

-- [문제 8] orders 테이블에서 3번 이상 주문된 도서의 book_id와 주문 횟수를 조회하세요.
select * from orders;
select distinct book_id from orders;
select distinct book_id, order_qty from orders order by book_id;

select book_id, count(*) from orders group by book_id;
select book_id, count(*) 주문회수 from orders group by book_id having 주문회수 >= 3;

-- [문제 9] orders 테이블에서 총 주문 수량이 5권을 초과하는 고객의 customer와 총 주문 수량을 조회하세요.
select customer, sum(order_qty) 주문수량 from orders group by customer;
select customer, sum(order_qty) 주문수량 from orders group by customer having 주문수량 >= 5 order by 주문수량 desc;
...
select * from books;
select * from books where genre = '컴퓨터';
select max(price) from books where genre = '컴퓨터';
select genre, max(price) from books group by genre having genre = '컴퓨터';
