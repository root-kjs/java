-- [SQL] 실습3 : SQL 실습 문제 ( 연산자 ) 250717(목)
...
select * from books where not genre in('소설' , '역사'); 
...
select title , author from books where not author like '이%';
...
select title , pub_date from books order by pub_date desc;
