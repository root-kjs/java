...
//[JAVA] 실습10 : 접근 제한자와 주요키워드 250710(목)
...
//3. main 함수에서 Member 객체를 생성하고, setId()로 아이디를 "admin"으로 설정한 뒤, getId()로 설정된 아이디를 가져와 출력하세요.
    ...
    // 1. 멤버변수와 메소드를 가져오기 위해 멤버 객체 생성
    Member member = new Member();
    // 2. id 저장/대입 메소드 가져오기
    member.setId( "admin" );
    // 3. id 가져오기 메소드 호출하여 화면 출력
...
//4. main 함수에서 Score 객체를 생성하고, setScore(85)와 setScore(120)를 각각 호출하여 결과가 올바르게 나오는지 확인하세요.
        ...
        score.setScore( 85 );
        // System.out.println( score.setScore( 85 )  ); 반환값이 없으니 에러
        score.setScore( 120 );
...
//4. main 함수에서 "123-456"을 계좌번호로 가진 객체를 생성하고, getAccountNumber()로 계좌번호를 출력하여 확인하세요.
         ...
         // 객체를 생성할때 계좌를 등록(초기화한다)
        BankAccount account = new BankAccount( "123-456" );

        // 리턴값을 가지고 있기 때문에  출력함수 사용해서 출력!
...
//2. main 함수에서 객체를 생성하지 않고, GameConfig.GAME_TITLE과 같이 클래스 이름으로 직접 접근하여 모든 설정 값을 출력하세요.
      ...
      //  GameConfig.GAME_TITLE; // 값인 경우에는 출력함수를 써야한다!
