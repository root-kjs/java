...
//[JAVA] 종합예제5 : boardService5 (메소드/역할 분리) 250709(수)
//[ 목표 ]
    ...
    public static void main(String[] args) {
        ...
        for( ; ; ) {// (1) 무한루프 : 프로그램 계속적으로 실행하기 위해서
            ...
            if( choose == 1 ) { // (4) 입력 에 따른 화면 출력
                ...
                // 1. 등록 메소드 호출하여 리턴값 변수 담고 리턴값에 따른 출력 구현
                ...
                Board[] result1 = bc.doGet();
                // 배열로 가져왔으니 배열 순회 자바의 랭스는 무조건 선언되어야 함. --> 왜? 메모리 공간을 미리 확보하기 위해서

                for ( Board board : result1 ) {

                    // 조회를 할때도 배열 선언되어 있다면 null안에서
                    // 조회하세요!
                    if ( board != null ) {
                        System.out.println("작성자 : " + board.writer);
                        System.out.println("내용 : " + board.content);
                        System.out.println("------------------------------------");
                        // 메소드 리턴값의 타입(클래스명)을 변수명 앞에 타입으로 가져온다.
                    }
                } // for end
