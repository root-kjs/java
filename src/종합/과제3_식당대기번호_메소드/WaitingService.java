...
//[JAVA] 종합과제3 : 식당 대기번호 프로그램 (메소드/역할 분리) 250708(화)
//[ 목표 ]
...
public class WaitingService {
    public static void main(String[] args) {
        ...
        // 콘트롤러 객체 생성
        WaitingController ws = new WaitingController();
        ...
        for(;;) {
            ...
            if( select == 1 ){
                ...
                // 사용자 입력값을 매개변수로 던져서 대기등록할 수 있는  콘트롤러 메소드 호출 하자!!!
                // 리턴값이 있으니 변수로 저장해서 가져올거야~
                boolean result = ws.addWaiting( phone , count );
                // 리턴값 변수의 상태에 따른 등록 결과를 출력하자!
                if( result == true ){
                ...
                // 조회 메소드 가져올거야~~ 조회 메소드 가져오려면 해당 메소드 결과를 담을 변수를 선언해줘야징.
                // 객체랑은 다른거야~ 타입은 메소드 타입 결과와 같은거로! 위에 콘트롤러 객체 이미 생성했으니 해당 객체와 메소드를 메소드 반환값 타입과 맞춰서 변수 선언해준다!
                // 배열 객체로 가져왔으니 목록으로 개별 출력하려면 포문을 돌려야겠지?
                // Waiting[] --> 의미는 Waiting 클래스가 배열로 여러개 가지고 있다는 거임? 오키??
                Waiting[] waitings = ws.getWaitingList();

                for( int i = 0; i <= waitings.length - 1; i++ ){
                    Waiting waiting = waitings[i];
                    //  해당 조회 배열 출력을 가져오면 빈배열이 아닌 얘들만 뿌려줘야 하니까 또 if 문
                    if( waiting != null ){
